...
def startpage():
	...
	text = """# OHSU Sentinel Node Prediction Tool for Sentinel Node Metastasis Risk
## DISCLAIMER

***Use of OHSU Sentinel Node Prediction Tool is subject to the following:***

1. The OHSU Sentinel Node Prediction Tool has been developed by researchers based on a published risk prediction model (see References)
...
3. Any clinician who uses the Calculator agrees and acknowledges that:
	...
	c. The information provided by the OHSU Sentinel Node Prediction Tool must only be used as a general guide. 

	d. The creators of the OHSU Sentinel Node Prediction Tool give no warranties, nor make any representations to any clinician or patient regarding the accuracy, completeness, timeliness or usefulness of any estimate or risk range generated by the OHSU Sentinel Node Prediction Tool; and

	e. The creators will have no liability to the clinician or any patient for the consequences of any treatment decision made - or any action taken or not taken - by the clinician or their patient following use of the OHSU Sentinel Node Prediction Tool."""
...
#import in the classifier and trained pipeline
	...
	st.title('Welcome to the OHSU Sentinel Node Prediction Tool')
...
app.add_app("The OHSU Sentinel Node Prediction Tool for Sentinel Node Metastasis Risk", app1) #Adds first page (app1) to the framework
