...
def num_pipeline(num_attribs, deg):
	return Pipeline([("select_numeric", DataFrameSelector(num_attribs)),('scaler', StandardScaler()),('poly', PolynomialFeatures(degree=deg))])

def cat_pipeline(cat_attribs):
	return Pipeline([("select_cat", DataFrameSelector(cat_attribs)),("cat_encoder", OneHotEncoder(sparse=False))])
